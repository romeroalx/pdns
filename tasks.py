        ...
        for distro in rpm_distros:
            for root, dirs, files in os.walk(f"{source}/{build_folder}"):
                for path in files:
                    ...
                    c.run(f'. {repo_home}/.venv/bin/activate && {pulp_cmd} rpm content -t package upload --file {source}/{file} --repository {repo_name} --no-publish')
            
            c.run(f'. {repo_home}/.venv/bin/activate && {pulp_cmd} file publication create --repository {repo_name}')
