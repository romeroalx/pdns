...
def get_base_configure_cmd_meson(build_dir, additional_c_flags='', additional_cxx_flags='', enable_systemd=True, enable_sodium=True):
    ...
    return " ".join([
        ...
        f"CC='{get_c_compiler()}'",
        f"CXX='{get_cxx_compiler()}'",
        f'pipenv run meson setup {build_dir}',
        # TODO
        # "--enable-systemd" if enable_systemd else '',
...
def ci_auth_run_unit_tests(c, meson=False):
    if meson:
        ...
        res = c.run('pipenv run meson test --verbose', warn=True)
    else:
...
def ci_make_install(c, meson=False):
    if meson:
        c.run('pipenv run ninja install --verbose')
    c.run('make install') # FIXME: this builds auth docs - again

@task
def install_meson(c):
    c.run(f'sudo apt-get install -y pipenv')
    c.run(f'pipenv install meson pyyaml ninja')

@task
def run_ninja(c):
    c.run(f'pipenv run ninja --verbose')
