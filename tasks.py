...
def install_dnsdist_test_deps(c, skipXDP=False): # FIXME: rename this, we do way more than apt-get
    ...
    libh2o_package_name = "libh2o-evloop0.13"
    ci_install_libh2o(c, libh2o_package_name)
...
def install_dnsdist_build_deps(c, skipXDP=False):
    ...
    libh2o_package_name = "libh2o-evloop-dev"
    ci_install_libh2o(c, libh2o_package_name)
...
@task
def ci_install_libh2o(c, package_name):
    res = c.run(f'apt-cache policy {package_name} | grep -qq Candidate && echo "True" || echo ""')

    if bool(res.stdout.strip()):
        c.run(f'sudo apt-get install -y {package_name}')
    else:
        build_and_install_h2o(c)
