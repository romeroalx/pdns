...
def pulp_upload_deb_packages_by_folder(repo_name, distribution_name, source):
    ...
    uploads the content without a repository associated, and then adds all objects to the
    repository to avoid blocking the resource in case of simultaneous uploads
    """

    upload_url = os.getenv("PULP_API_URL", "") + "/pulp/api/v3/content/deb/packages/"
    ...
    for root, dirs, files in os.walk(source):
        for file in files:
            if file.endswith(".deb"):
                full_path = os.path.join(root, file)
                cmd = f"artifact upload --file {full_path} --chunk-size 500MB"

                artifact = run_pulp_cmd(cmd)
                artifact_href = get_pulp_output_attribute(artifact, "pulp_href")
                ...
                try:
                    res = requests.post(
                        upload_url, auth=auth, headers=headers, json=package_data
                    )
                    res.raise_for_status()
                except requests.exceptions.HTTPError as e:
                    print(f"::error::Error creating DEB upload: {e.stderr.strip()}")
                    sys.exit(1)

                # pass output and the function converts it to json
                tasks.append(get_pulp_output_attribute(res.text, "task"))

    # Check all tasks have been completed before publishing the latest version
    for task in tasks:
        if not is_pulp_task_completed(task):
            print(
                f"::error::Error uploading DEB packages into Pulp. Task {task} timeout"
            )
            sys.exit(1)
